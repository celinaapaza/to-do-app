...
void main(List<String> text) async {
  ...
  await viewFile.writeAsString(
    _writeView(fileName: fileName, className: className),
  );
  await viewFile.create();

  File controllerFile = File(
    "lib/src/ui/page_controllers/${fileName}_page_controller.dart",
  );
...
String _writeView({required String fileName, required String className}) {
  return """
...
class ${"${className}PageController"} extends ControllerMVC implements IPageController {
